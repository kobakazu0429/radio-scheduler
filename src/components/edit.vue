...
module.exports = {
  methods: {
    ...
    send: function() {
      ...
      let tmp_drew_thumbnail =
        nowDrew_thumbnail == 0 || nowDrew_thumbnail == false ? 0 : 1;
      ...
      params.append('time', this.$store.state.now_form_time);
      params.append('title', this.$store.state.now_form_title);
      params.append('manager', this.$store.state.now_form_manager);
      ...
      this.axios
        .patch('http://localhost:3000/api/v2/tasks/' + thisId + '/', params)
        .then(response => {
          location.reload();
        })
        .catch(error => {
